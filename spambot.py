...
def new_member(bot, update):
    for member in update.message.new_chat_members:
        if member.username == 'Kolya71Bot':
            with open("/web/kolya-bot/data/chat_ids.txt", "a") as chats_file:
                chats_file.write(str(update.message.chat.id))
            with open("/web/kolya-bot/data/chat_infos.txt", "a") as chats_file:
...
def new_post(bot, update):
    if update.message.from_user.id in ALLOWED_USERS:
        with open("/web/kolya-bot/data/chat_ids.txt", "r") as chats_file:
...
def added_chats(bot, update):
    if update.message.from_user.id in ALLOWED_USERS:
        with open("/web/kolya-bot/data/chat_infos.txt", "r") as chats_infos_file:
